...
namespace BL
{
    ...
    public class Shapes
    {
        ...
        public static Queue<pictureMode> CreatShape(int hightKolaz, int widthKolaz, int numPicture, int space, Image[] imageArr, string shapeName)
        {
            try {
            string filePicture = @"C:/Users/ptzadkax/OneDrive - Intel Corporation/Desktop/אנגולר23/angular-starter/angular-starter/src/shape" + shapeName + ".png";
            Bitmap imgShape = new Bitmap(filePicture);
            int[,] mat = new int[sizeOfMat, sizeOfMat];
            int[,] ezer = new int[sizeOfMat, sizeOfMat];
            Bitmap img = new Bitmap(sizeOfMat, sizeOfMat);
            ...
            graph.DrawImage(imgShape, 0, 0, sizeOfMat, sizeOfMat);
            graph.Dispose();
            mat = getFixelToMat(img);
            ezer = mat;
            minAndMax[] arrMinAndMax = divToGroup(numPicture, mat, ezer, sizeOfMat, sizeOfMat);
            return changeArrPointToQueue(arrMinAndMax);
            }
            catch
            {
                return null;
        ...
        public static Queue<pictureMode> changeArrPointToQueue(minAndMax [] arrMinAndMax)//return queue with pointStart and pointEnd
            {
               pictureMode p;
               Queue<pictureMode> pictureQueue = new Queue<pictureMode>();
               for (int i = 0; i < arrPoint.Length; i++)
               {
                   Point pointStart = new Point(arrPoint[i].X-((arrMinAndMax[i].xMax - arrMinAndMax[i].xMin) / 2), arrPoint[i].Y-(arrMinAndMax[i].yMax - arrMinAndMax[i].yMin) / 2);
                   Point pointEnd = new Point(arrPoint[i].X +((arrMinAndMax[i].xMax - arrMinAndMax[i].xMin) / 2), arrPoint[i].Y + (arrMinAndMax[i].yMax - arrMinAndMax[i].yMin) / 2);              
                   p.pointStart = pointStart;
                   p.pointEnd = pointEnd;
                   p.mode=Convert.ToBoolean(null);
                   pictureQueue.Enqueue(p);
               }
               return pictureQueue;
            }
    }
